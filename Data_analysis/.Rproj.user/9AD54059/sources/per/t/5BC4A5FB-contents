...
onlyCross <- F                        # use only cross validation to select model
...
logVars <- c( "_Y", "_C", "_I" )
aggrVars <- append( logVars, c( "_Lev", "_FFI", "_PDIndex", "_ProfitRate", "_P", "_KN", "_Minskian", "HHI", "LumH", 
                                "Markup", "u", "ProfitRate", "MedA", "MinA", "TechHet", "TD", "gY" ) )
newVars <- c( "llev", "invRate", "regCoef" ) #"freq_FE", "cris_llh", "GDP_sd"
varsRemove <- c( "_Y", "_C", "_I", "_P", "_KN" )
...
if( ! exists( "plot_norm", mode = "function" ) )             # already loaded?
  source( "./Support_Functions/other-support-functions.R" )
...
eval.vars <- function( data, vars ) {
  ...
  colnames( data ) <- stringr::str_remove( string = colnames( data ), "_" )
  ...
  for( var in vars ) {
    ...
    } else if( var == "llev" ) {   # add variable of lagged leverage
      data[ , "llev" ] <- lag( data[ , "Lev" ] )
      
    } else if( var == "invRate" ) {   # Calculate investment rate
      data[ , "invRate" ] <- data[ , "P" ] * data[ , "I" ] / lag( data[, "KN" ] ) 
    
    } else if( var == "regCoef" ) {   # add the GDP std. dev. variable to data set
      subData <- as.data.frame( data[ , c( "FFI", "llev",  "ProfitRate", "invRate", "PDIndex" ) ] )
      reg <- lm( FFI ~ 1 + llev + ProfitRate + invRate + PDIndex, data = subData )
      data[ , var ] <- reg$coefficients[5]
    }
  }

  data <- data[, !colnames( data ) %in% varsRemove ]
...
# Adequate the name of variables to the paper's
rownames(SA$sa) <- ifelse( rownames(SA$sa) == 'gammau', 'rhou', ifelse( rownames(SA$sa) == 'n', 'F', rownames(SA$sa) ) )
...
# Adequate the name of variables to the paper's
colnames(data$doe) <- ifelse( colnames(data$doe) == 'gammau', 'rhou', ifelse( colnames(data$doe) == 'n', 'F', colnames(data$doe) ) )
