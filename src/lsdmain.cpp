...
bool ignore_eq_file =
    true; // flag to ignore equation file in configuration file
...
// int strWindowOn = true;		// control the presentation of the model
// structure window (bool)
int strWindowOn =
    false; // control the presentation of the model structure window (bool)
...
bool meta_par_in[META_PAR_NUM]; // flag meta parameter for simulation settings
                                // found
...
bool user_exception =
    false;    // flag indicating exception was generated by user code
...
char watch_elem[MAX_ELEM_LENGTH + 1] =
    "";                    // name of element triggering watch condition
...
const char lsdCmdHlp[] =
    "Command line options:\n'-f FILENAME.lsd [-s SEED] [-e RUNS] to run a "
    "single configuration file\n'-f FILE_BASE_NAME -s FIRST_NUM [-e LAST_NUM]' "
    "for batch sequential mode\n'-o PATH' to save result file(s) to a "
    "different subdirectory\n'-l FILENAME' to save all output to a (log) "
    "file\n'-t' to produce comma separated (.csv) text result file(s)\n'-r' "
    "for skipping the generation of intermediate result file(s)\n'-p' for "
    "skipping the generation of totals file\n'-g' for the generation of a "
    "single grand total file\n'-z' for preventing the generation of compressed "
    "result file(s)\n'-b' for showing a progress bar\n'-c "
    "MAX_THREADS[:MAX_RUNS]' to set maximum parallel threads/runs to use\n";
#endif
...
int lsdmain(int argn, const char **argv) {
  ...
  strcpy(simul_name, "");
  exec_path =
      getcwd(exec_path, MAX_PATH_LENGTH); // assume exec path is current path
  exec_file =
      clean_file(argv[0]); // global pointer to the name of executable file
  exec_path =
      clean_path(exec_path); // global pointer to path of executable file
  ...
  if (exec_file == NULL || exec_path == NULL) {
    fprintf(stderr,
            ...
            "directory is not too deep into the disk directory tree (over %d "
            "chars).\n\n",
  ...
  if (argn < 3) {
    ...
    for (i = 1; i < argn; i += 2) {
      ...
      if (argv[i][0] == '-' && argv[i][1] == 'f' && 1 + i < argn &&
          strlen(argv[1 + i]) > 0) {
        delete[] simul_name;
      ...
      if (argv[i][0] == '-' && argv[i][1] == 'g') {
        i--; // no parameter for this option
        grandTotal = true;
        printf("\nGrand total file requested ('-g'), don't run another "
               "instance of 'lsdNW' in this folder!\n");
        continue;
      }
  ...
  if ((f = fopen(struct_file, "r")) == NULL) {
    fprintf(stderr,
            "\nFile '%s' not found.\nThis is the no window version of "
            "LSD.\nSpecify a -f FILENAME.lsd to run a simulation or -f "
            "FILE_BASE_NAME -s 1 for\nbatch sequential simulation mode "
            "(requires configuration files:\nFILE_BASE_NAME_1.lsd, "
            "FILE_BASE_NAME_2.lsd, etc).\n\n",
            struct_file);
    myexit(7);
  }
  ...
  if (load_configuration(true, 1) != 0) {
    fprintf(stderr,
            "\nFile '%s' is invalid.\nThis is the no window version of "
            "LSD.\nCheck if the file is a valid LSD configuration or "
            "regenerate it using the\nLSD Browser.\n\n",
            struct_file);
    myexit(8);
  }
  ...
  if (log_filename != NULL) {
    if (save_alt_path &&
        strncmp(log_filename, alt_path, strlen(alt_path)) != 0) {
      str = log_filename;
  ...
  if (!batch_sequential && sim_num > 1 && max_runs > 1) {
    if (grandTotal || !no_tot) {
      printf("\n(Grand) total file(s) request ignored, running in parallel "
             "mode.\n");
      no_tot = true;
      grandTotal = false;
    }

    return run_parallel(no_window, argv[0], simul_name, seed, sim_num,
                        max_threads, max_runs);
  }
  ...
  for (i = 1; argv[i] != NULL; i++) {
    if (exec_file == NULL || exec_path == NULL) {
      log_tcl_error(true, "Invalid LSD executable name or path",
                    "Make sure the LSD directory is not too deep into the disk "
                    "directory tree");
      myexit(1);
    }

    if (argv[i][0] != '-' ||
        (argv[i][1] != 'f' && argv[i][1] != 'i' && argv[i][1] != 'c')) {
      log_tcl_error(true, "Command line parameters",
                    "Invalid option, available options: -i TCL_DIRECTORY / -f "
                    "MODEL_NAME / -c MAX_THREADS");
      myexit(1);
    }
  ...
  if (f == NULL || strlen(exec_path) == 0 ||
      !strcmp(exec_path, "/")) { // try to get name from Tcl
    cmd("if { [ info nameofexecutable ] != \"\" } { set path [ file dirname [ "
        "info nameofexecutable ] ] } { set path \"\" }");
    app = get_str("path");
  ...
  // check if LSDROOT already exists and use it if so, if not, search the
  // current directory tree
  cmd("if [ info exists env(LSDROOT) ] { set RootLsd [ file normalize "
      "$env(LSDROOT) ]; if { ! [ file exists \"$RootLsd/src/interf.cpp\" ] } { "
      "unset RootLsd } }");
  ...
  if (choice) {
    log_tcl_error(false, "LSDROOT check",
                  "LSDROOT not set, make sure the environment variable LSDROOT "
                  "points to the directory where LSD is installed");
    cmd("tk_messageBox -parent . -title Error -icon error -type ok -message "
        "\"LSDROOT not set\" -detail \"Please make sure the environment "
        "variable LSDROOT points to the directory where LSD is "
        "installed.\n\nLSD is aborting now.\"");
    myexit(9);
  }
  ...
  if (app != NULL && strlen(app) > 0) {
    ...
    log_tcl_error(false, "LSD directory check",
                  "Cannot locate LSD folder on disk, check the installation of "
                  "LSD and reinstall LSD if the problem persists");
    cmd("tk_messageBox -parent . -title Error -icon error -type ok -message "
        "\"LSD directory missing\" -detail \"Cannot locate the LSD "
        "installation folder on disk.\nPlease check your installation and "
        "reinstall LSD if the problem persists.\n\nLSD is aborting now.\"");
    myexit(9);
  }
  ...
  // load required Tcl/Tk data, procedures and packages (error coded by file/bit
  // position)
  choice = 0;

  // load native Tk procedures for graphical user interface management
  cmd("if [ file exists \"$RootLsd/$LsdSrc/gui.tcl\" ] { if [ catch { source "
      "\"$RootLsd/$LsdSrc/gui.tcl\" } err0x01 ] { set choice [ expr { $choice "
      "+ %d } ] } } { set choice [ expr { $choice + %d } ] }",
      0x0100, 0x01);

  // load native Tcl procedures for general utilities
  cmd("if [ file exists \"$RootLsd/$LsdSrc/file.tcl\" ] { if [ catch { source "
      "\"$RootLsd/$LsdSrc/file.tcl\" } err0x02 ] { set choice [ expr { $choice "
      "+ %d } ] } } { set choice [ expr { $choice + %d } ] }",
      0x0200, 0x02);

  // load additional native Tcl procedures for external files handling
  cmd("if [ file exists \"$RootLsd/$LsdSrc/util.tcl\" ] { if [ catch { source "
      "\"$RootLsd/$LsdSrc/util.tcl\" } err0x04 ] { set choice [ expr { $choice "
      "+ %d } ] } } { set choice [ expr { $choice + %d } ] }",
      0x0400, 0x04);

  if (choice != 0) {
    log_tcl_error(false, "Source files check failed",
                  "Required Tcl/Tk source file(s) missing or corrupted "
                  "(0x%04x), check your installation and reinstall LSD if the "
                  "problem persists\n\n0x01: %s\n\n0x02: %s\n\n0x04: %s",
                  choice, get_str("err0x01"), get_str("err0x02"),
                  get_str("err0x04"));
    cmd("tk_messageBox -parent . -title Error -icon error -type ok -message "
        "\"File(s) missing or corrupted\" -detail \"Some critical Tcl files "
        "(0x%04x) are missing or corrupted.\nPlease check your installation "
        "and reinstall LSD if the problem persists.\n\nLSD is aborting now.\"",
        choice);
    myexit(200 + choice);
  }
  ...
  if (!strcmp(app, "linux"))
    platform = _LIN_;
  else if (!strcmp(app, "mac"))
    platform = _MAC_;
  else if (!strcmp(app, "windows"))
    platform = _WIN_;
  else {
    log_tcl_error(false, "Unsupported platform",
                  "Your computer operating system is not supported by this LSD "
                  "version, you may try an older version compatible with "
                  "legacy systems (Windows 32-bit, Mac OS X, etc.)");
    cmd("ttk::messageBox -parent . -type ok -icon error -title Error -message "
        "\"Unsupported platform\" -detail \"Your computer operating system is "
        "not supported by this LSD version,\nyou may try an older version "
        "compatible with legacy systems\n(Windows 32-bit, Mac OS X, "
        "etc.)\n\nLSD is aborting now.\"",
        choice);
    myexit(200);
  }
  ...
  // create a Tcl command that calls the C discard_change function before
  // killing LSD
  ...
  Tcl_CreateCommand(inter, "abort_run_threads", Tcl_abort_run_threads, NULL,
                    NULL);
  ...
  if (eval_bool("[ info exists lastConf ] && [ file exists $lastConf ] && [ "
                "file isfile $lastConf ]")) {
    delete[] simul_name;
    cmd("set fn [ string map -nocase [ list [ file extension $lastConf ] \"\" "
        "] [ file tail $lastConf ] ]");
  ...
  if (strlen(simul_name) > 0) {
    ...
    sprintf(struct_file, "%s%s%s.lsd", path, strlen(path) > 0 ? "/" : "",
            simul_name);
...
void run(void) {
  ...
  char *path_out = NULL, *name_out, sep_out[2], fname[MAX_PATH_LENGTH],
       bar_done[2 * BAR_DONE_SIZE];
  ...
  if (search_parallel(root))
    plog("\nWarning: parallel mode is not supported under current "
         "configuration\n");
  parallel_mode = false;
#endif
  ...
  cover_browser(
      ...
      "simulation\n'Pause' / 'Resume' :  pauses and resumes the "
      "simulation\n'Fast' :	accelerates the simulation by hiding "
      "information\n'Observe' :  presents more run-time information\n'Debug' : "
      " triggers the debugger at flagged variables",
      true);
#else
  ...
  for (i = 1, quit = 0; i <= sim_num && quit != 2; ++i) {
    ...
    if (fast_mode < 2) {
      if (parallel_mode)
        plog("\nSimulation %d of %d running (seed=%d threads=%d)...", i,
             sim_num, seed, max_threads);
      else
    ...
    if (batch_sequential_loop) {
      if (load_configuration(true, 1) != 0) {
#ifndef _NW_
        log_tcl_error(true, "Load configuration",
                      "Configuration file not found or corrupted");
        cmd("ttk::messageBox -parent . -type ok -icon error -title Error "
            "-message \"Configuration file cannot be loaded\" -detail \"Check "
            "if LSD still has WRITE access to the configuration file "
            "'%s'.\nLSD will close now.\"",
            struct_file);
#else
    ...
    if (i > 1)
      if (load_configuration(true, 2) != 0) {
        ...
        log_tcl_error(true, "Load configuration",
                      "Configuration file not found or corrupted");
        cmd("ttk::messageBox -parent . -type ok -icon error -title Error "
            "-message \"Configuration file cannot be reloaded\" -detail "
            "\"Check if LSD still has WRITE access to the configuration file "
            "'%s'.\nLSD will close now.\"",
            struct_file);
#else
        fprintf(stderr, "\nFile '%s' not found or corrupted.\n", struct_file);
    ...
    if (!alloc_save_mem(root)) {
      ...
      cmd("ttk::messageBox -parent . -type ok -icon error -title Error "
          "-message \"Not enough memory\" -detail \"Too many series saved for "
          "the available memory. Memory insufficient for %d series over %d "
          "time steps. Reduce series to save and/or time steps.\nLSD will "
          "close now.\"",
          series_saved, max_step);
#else
      fprintf(stderr,
              "\nNot enough memory. Too many series saved for the memory "
              "available.\nMemory insufficient for %d series over %d time "
    ...
    for (t = 1; quit == 0 && t <= max_step; ++t) {
      ...
      case 9: // pause simulation
        pause_run = !pause_run;
        if (pause_run) {
          cmd("set origLogTit [ wm title .log ]; wm title .log \"$origLogTit "
              "(PAUSED)\"");
      ...
      if (((float)clock() - last_update) / CLOCKS_PER_SEC > UPD_PER) {
        ...
        cmd(".p.b2.i configure -text \"Case: %d of %d ([ expr { int( 100 * %d "
            "/ %d ) } ]%% done)\"",
            min(t + 1, max_step), max_step, t, max_step);
        cmd("update");
        last_update = clock();
      }
#endif
    ...
    cmd(".p.b1.i configure -text \"Simulation: %d of %d ([ expr { int( 100 * "
        "%d / %d ) } ]%% done)\"",
        min(cur_sim + 1, sim_num), sim_num, cur_sim, sim_num);
    ...
    if (quit != 2 && (sim_num > 1 || no_window)) {
      ...
      if (series_saved > 0) { // remove existing path, if any, from name in case
                              // of alternative output path
        ...
        if (!no_res) {
          if (!batch_sequential)
            snprintf(fname, MAX_PATH_LENGTH, "%s%s%s_%d.%s", path_out, sep_out,
                     name_out, seed - 1, docsv ? "csv" : "res");
          else
            snprintf(fname, MAX_PATH_LENGTH, "%s%s%s_%d_%d.%s", path_out,
                     sep_out, name_out, findex, seed - 1,
                     docsv ? "csv" : "res");
          ...
          rf = new result(fname, "wt", dozip,
                          docsv);          // create results file object
        ...
        if (!no_tot && (!no_window || max_runs == 1)) {
          if (!grandTotal || batch_sequential) // generate partial total files?
          {
            if (!batch_sequential)
              snprintf(fname, MAX_PATH_LENGTH, "%s%s%s_%d_%d.%s", path_out,
                       sep_out, name_out, seed - i, seed - 1 + sim_num - i,
                       docsv ? "csv" : "tot");
            else
              snprintf(fname, MAX_PATH_LENGTH, "%s%s%s_%d_%d_%d.%s", path_out,
                       sep_out, name_out, findex, seed - i,
                       seed - 1 + sim_num - i, docsv ? "csv" : "tot");
          } else // generate single grand total file
          {
            snprintf(fname, MAX_PATH_LENGTH, "%s%s%s.%s", path_out, sep_out,
                     name_out, docsv ? "csv" : "tot");
          }
          ...
          if (i == 1 && grandTotal && !add_to_tot) {
            ...
            rf = new result(fname, "a", dozip,
                            docsv); // add results object to existing file
      ...
      if (i == sim_num) // last run?
      {
        if (batch_sequential) // last batch file?
        {
          ...
          if (f == NULL ||
              (fend != 0 && findex > fend)) // no more file to process
          {
            if (f != NULL)
              fclose(f);
            if (fast_mode < 2)
...
SET_VAR
*********************************/
// function to set a c variable when not in a Tcl idle loop (hardcoded vars
// only)
#ifndef _NW_
int Tcl_set_c_var(ClientData cdata, Tcl_Interp *inter, int argc,
                  const char *argv[]) {
...
void empty_stack(void) {
  if (stacklog != NULL) {
    ...
    cmd("ttk::messageBox -parent . -type ok -icon error -title Error -message "
        "\"Internal LSD error\" -detail \"The LSD trace stack is "
        "corrupted.\nLSD will close now.\"");
#else
  ...
  for (cv = r->v; cv != NULL; cv = cv->next) {
    if ((cv->num_lag > 0 || cv->param == 1) && cv->data_loaded == '-') {
      error_hard("required initialization values missing",
                 ...
                 false, "%s '%s' in object '%s' has not been initialized",
                 cv->param == 1 ? "parameter" : "variable", cv->label,
                 r->label);
      toquit = 2;
    }
  ...
  if (!no_more_memory) {
    ...
    if (v->data == NULL) {
      ...
      if (no_more_memory != prev_state) {
        set_lab_tit(v);
        plog("\nWarning: cannot allocate memory for saving '%s %s' (object "
...
***************************************/
#define INISTAT -1234
int run_parallel(bool nw, const char *exec, const char *simname, int fseed,
                 int runs, int thrrun, int parruns) {
  char *alt_name;
  int i, j, k, num, sl;
  ...
  if (runs > parruns) // more than one run per thread?
  {
    ...
    for (i = fseed, j = 1; j <= parruns; ++j) {
      // log file name
      snprintf(log_file, log_len, "%s%s%s_%d.log",
               save_alt_path ? alt_path : path,
               strlen(save_alt_path ? alt_path : path) > 0 ? "/" : "",
               save_alt_path ? alt_name : simname, j);
      run_logs.push_back(log_file);

      // results file names
      for (k = i; k < i + num + (j <= sl ? 1 : 0); ++k) {
        snprintf(res_file, res_len, "%s%s%s_%d.%s",
                 save_alt_path ? alt_path : path,
                 strlen(save_alt_path ? alt_path : path) > 0 ? "/" : "",
                 save_alt_path ? alt_name : simname, k, docsv ? "csv" : "res");
      ...
      // command line
      snprintf(cmd, cmd_len, "%s -c %d -f %s.lsd -s %d -e %d%s%s%s%s%s%s -l %s",
               exec, thrrun, simname, i, j <= sl ? num + 1 : num,
               no_res ? " -r" : "", no_tot ? " -p" : "", docsv ? " -t" : "",
               dozip ? "" : " -z", dobar ? " -b" : "", dest_path, log_file);

      run_pids.resize(run_pids.size() + 1);
      run_status.push_back(INISTAT);
      run_threads.push_back(
          thread(run_parallel_exec, nw, run_status.size() - 1, string(cmd)));
  ...
  } else // just one run per thread
  {
    for (i = fseed, j = 1; i < fseed + runs; ++i, ++j) {
      ...
      snprintf(log_file, log_len, "%s%s%s_%d.log",
               save_alt_path ? alt_path : path,
               strlen(save_alt_path ? alt_path : path) > 0 ? "/" : "",
               save_alt_path ? alt_name : simname, i);
      run_logs.push_back(log_file);
      ...
      snprintf(res_file, res_len, "%s%s%s_%d.%s",
               save_alt_path ? alt_path : path,
               strlen(save_alt_path ? alt_path : path) > 0 ? "/" : "",
               save_alt_path ? alt_name : simname, i, docsv ? "csv" : "res");
      ...
      // command line
      snprintf(cmd, cmd_len, "%s -c %d -f %s.lsd -s %d -e 1%s%s%s%s%s%s -l %s",
               exec, thrrun, simname, i, no_res ? " -r" : "",
               no_tot ? " -p" : "", docsv ? " -t" : "", dozip ? "" : " -z",
               dobar ? " -b" : "", dest_path, log_file);
    ...
    for (string &log : run_logs) {
      ...
      if (f == NULL) {
        snprintf(buf, MAX_LINE_SIZE,
                 "\nCannot read '%s', consolidated log is incomplete",
                 log.c_str());
        run_log.append(buf);
        continue;
      }
...
void update_bar(char *bar, int done, int &last_done, int bar_sz) {
  ...
  for (p = last_done + 1; p <= done; ++p)
    ...
    } else if (p % (100 / (BAR_DONE_SIZE - 33)) == 0) {
      if (bar != NULL)
        strcatn(bar, ".", bar_sz);
...
void create_logwindow(void) {
  ...
  cmd("newtop .log \"LSD Log\" { if { [ discard_change ] eq \"ok\" && [ "
      "abort_run_threads ] eq \"ok\" } { exit } } \"\"");

  cmd("set w .log.text");
  cmd("ttk::frame $w");
  cmd("ttk::scrollbar $w.scroll -command \"$w.text yview\"");
  cmd("ttk::scrollbar $w.scrollx -command \"$w.text xview\" -orient hor");
  cmd("ttk::text $w.text -yscrollcommand \"$w.scroll set\" -xscrollcommand "
      "\"$w.scrollx set\" -wrap none -entry 0 -dark $darkTheme -style "
      "smallFixed.TText");
  ...
  cmd("$w.text.menu add command -label Copy -underline 0 -accelerator Ctrl+C "
      "-command { tk_textCopy .log.text.text }"); // entryconfig 0
  cmd("$w.text.menu add command -label Clear -accelerator Ctrl+Del -command { "
      ".log.text.text.internal delete 0.0 end }"); // entryconfig 1
  cmd("$w.text.menu add separator");               // entryconfig 2
  cmd("$w.text.menu add command -label Help -accelerator F1 -command { LsdHelp "
      "log.html }"); // entryconfig 3
  ...
  cmd("bind .log <Control-c> { .log.text.text.menu invoke 0 }; bind .log "
      "<Control-C> { .log.text.text.menu invoke 0 }");
  cmd("bind .log <Control-Delete> { .log.text.text.menu invoke 1 }");

  // replace text widget default insert, delete and replace bindings, preventing
  // the user to change it
  cmd("rename .log.text.text .log.text.text.internal");
  cmd("proc .log.text.text { args } { switch -exact -- [ lindex $args 0 ] { "
      "insert { } delete { } replace { } default { return [ eval "
      ".log.text.text.internal $args] } } }");

  cmd("plog \"LSD Version %s (%s)\nCopyright Marco Valente and Marcelo "
      "Pereira\nLSD is distributed under the GNU General Public License\nLSD "
...
void set_shortcuts_run(const char *window) {
  if (exists_window(window)) {
    cmd("bind %s <KeyPress-s> { catch { .b.r2.stop invoke } }; bind %s "
        "<KeyPress-S> { catch { .b.r2.stop invoke } }",
        window, window);
    cmd("bind %s <KeyPress-p> { catch { .b.r2.pause invoke } }; bind %s "
        "<KeyPress-P> { catch { .b.r2.pause invoke } }",
        window, window);
    cmd("bind %s <KeyPress-r> { catch { .b.r2.pause invoke } }; bind %s "
        "<KeyPress-R> { catch { .b.r2.pause invoke } }",
        window, window);
    cmd("bind %s <KeyPress-f> { catch { .b.r2.speed invoke } }; bind %s "
        "<KeyPress-F> { catch { .b.r2.speed invoke } }",
        window, window);
    cmd("bind %s <KeyPress-o> { catch { .b.r2.obs invoke } }; bind %s "
        "<KeyPress-O> { catch { .b.r2.obs invoke } }",
        window, window);
    cmd("bind %s <KeyPress-d> { catch { .b.r2.deb invoke } }; bind %s "
        "<KeyPress-D> { catch { .b.r2.deb invoke } }",
        window, window);
  }
}
...
void cover_browser(const char *text1, const char *text2, bool run) {
  ...
  cmd("ttk::frame .t1");
  cmd("ttk::label .t1.l1 -justify center -text \"%s\" -style bold.TLabel",
      text1);
  ...
  if (run) {
    ...
    cmd("ttk::label .p.b1.i -text \"Simulation: 1 of %d (0%% done)\" -anchor "
        "center",
        sim_num);
    ...
    cmd("ttk::label .p.b2.i -text \"Case: 1 of %d (0%% done)\" -anchor center",
        max_step);
  ...
  if (run) {
    ...
    cmd("ttk::frame .b");
    cmd("ttk::frame .b.r2");
    cmd("ttk::button .b.r2.stop -width $goWid -text Stop -command { set_c_var "
        "done_in 1 } -underline 0");
    cmd("ttk::button .b.r2.pause -width $goWid -text Pause -command { "
        "set_c_var done_in 9 } -underline 0");
    cmd("ttk::button .b.r2.speed -width $goWid -text Fast -command { set_c_var "
        "done_in 2 } -underline 0");
    cmd("ttk::button .b.r2.obs -width $goWid -text Observe -command { "
        "set_c_var done_in 4 } -underline 0");
    cmd("ttk::button .b.r2.deb -width $goWid -text Debug -command { set_c_var "
        "done_in 3 } -underline 0");
    cmd("pack .b.r2.stop .b.r2.pause .b.r2.speed .b.r2.obs .b.r2.deb -padx "
        "$butSpc -side left");
    cmd("pack .b.r2");
...
void show_prof_aggr(void) {
  ...
  for (it1 = vars.begin(); it1 != vars.end(); ++it1)
    plog_tag("\n%-12.12s\t%-12.12s\t%d\t%d", "prof2", it1->obj, it1->var,
             it1->time, it1->count);
