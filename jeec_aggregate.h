...
/* Sum investment and consumption */
	...
	// In the theoretical case demand is higher than supply, reduce government expenditures
	v[5] = v[4] - v[0] - v[1];
	v[5] = max( 0, v[5] );
	cur = SEARCH( "Government" );
	WRITES( cur, "GovernmentConsumption", v[5] );

	// Check whether the reduction of government consumption suffice to match aggregate demand and aggregate supply
	v[6] = v[0] + v[1] + v[5];
	if ( v[4] > v[6] )
		END_EQUATION( v[6] );

	// To assure all possible cases, if demand is still higher than supply, reduce households' consumption
	v[7] = v[4] - v[1] - v[5];
	v[8] = max( v[7], 0 );
...
EQUATION( "AggregateDebtFirms" )
	...
	CYCLE( cur, "Sectors" )
	{
		CYCLES( cur, cur1, "Firms" )
		{
			v[0] += VS( cur1, "TotalDebt" );
		}
	}
RESULT( v[0] )

EQUATION( "AggregateFirmDeposits" )
	/*CYCLE( cur, "Sectors" )										// run after entry takes place
	{
		VS( cur, "Entry" );
	}*/
...
/*
The financial fragility index is calculated according to the financial status of firms.
If a firm is hedge, it weights nothing in the index.
A speculative firm weights 0.5.
A Ponzi firm weights 1.
The weight of a single firm in the index is given by the overall share of assets.
Level: country.
*/
...
/*Proportion of speculative firms. Not-weighted.*/
	...
	CYCLE( cur, "Sectors" )
	{
		CYCLES( cur, cur1, "Firms" )
		{
			v[0] += VS( cur1, "SpeculativeFirmFlag" );
		}
	}
	v[2] = v[0] / v[1];
RESULT( v[2] )
...
/*Growth rate of best-known technology of firms, weighted by the market shares.*/
RESULT( WHTAVE( "ACurrent", "MarketShare" ) / WHTAVEL( "ACurrent", "MarketShare", 1 ) - 1 )
...
RESULT( VL( "AggregateDebtFirms", 1 ) > 0 ? SUM( "InterestPayment" ) / VL( "AggregateDebtFirms", 1 ) : 0 )
