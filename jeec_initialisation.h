...
*/
	...
	// for some reason, when running in parallel, CONFIG macro returns the complete address of the configuration file
	// in this case, this chunk of the code removes the address from the string, include a dot in rPath address and also rewrite pPath.
	...
	std::string fullCommand = "Rscript " + addressScript + " " + rSave + " " + scriptLocation;
	...
	WRITELLS( cur, "DesiredCapacityUtilisation", imp_data( "uT", scriptLocation, rSave ), 0, 1 );
	...
	WRITELLS( cur, "NominalSales", imp_data( "S", scriptLocation, rSave ) / v[2] * imp_data( "P", scriptLocation, rSave ), 0, 1 );
	WRITELLS( cur, "ExpectedSales", imp_data( "S", scriptLocation, rSave ) / v[2], 0, 1 );
	WRITELLS( cur, "FirmDemand", imp_data( "S", scriptLocation, rSave ) / v[2], 0, 1 );
	WRITELLS( cur, "FirmDeposits", imp_data( "Df", scriptLocation, rSave ) / v[2], 0, 1 );
	WRITELLS( cur, "MarketShare", 1 / v[2], 0, 1 );
	WRITELLS( cur, "MarketShare", 1 / v[2], 0, 2 );
	WRITELLS( cur, "UnitCost", imp_data( "w", scriptLocation, rSave ) / imp_data( "A", scriptLocation, rSave ), 0, 1 );
	WRITELLS( cur, "Leverage", VS( root, "lev0" ), 0, 1 );
	WRITELLS( cur, "AvgLabourProductivity", imp_data( "A", scriptLocation, rSave ), 0, 1 );
	...
	WRITELLS( cur3, "HouseholdDeposits", imp_data( "Dh", scriptLocation, rSave ), 0, 1 );
	...
	WRITELLS( cur5, "NetWorthBank", imp_data( "nwB", scriptLocation, rSave ) / V( "nBanks" ), 0, 1 );
	WRITELLS( cur5, "LoanPortfolio", imp_data( "L", scriptLocation, rSave ) / V( "nBanks" ), 0, 1 );
	WRITELLS( cur5, "BillsBank", imp_data( "Bb", scriptLocation, rSave ) / V( "nBanks" ), 0, 1 );
	WRITELLS( cur5, "DepositsBank", ( imp_data( "Df", scriptLocation, rSave ) + imp_data( "Dh", scriptLocation, rSave ) )  / V( "nBanks" ), 0, 1 );
	WRITELLS( cur5, "AverageBankInterestRate", VS( cur4, "BaseInterestRate" ) + imp_data( "spread", scriptLocation, rSave ), 0, 1 );
	...
	CYCLE( cur, "Countries" )
	{
		...
		WRITELLS( cur, "AggregateCapacityUtilisation", imp_data( "uT", scriptLocation, rSave ), 0, 1 );
		WRITELLS( cur, "AggregateInventories", imp_data( "INV", scriptLocation, rSave ), 0, 1 );
		WRITELLS( cur, "AggregateFirmDeposits", imp_data( "Df", scriptLocation, rSave ), 0, 1 );
		WRITELLS( cur, "TotalLabour", VS( root, "Nf0" ), 0, 1 );
		WRITELLS( cur, "GrowthRateEmployment", imp_data( "gss", scriptLocation, rSave ), 0, 1 );
		WRITELLS( cur, "AggregateGrowthProductivity", VS( root, "gk0" ), 0, 1 );
		WRITELLS( cur, "AggregateProductivity", imp_data( "A", scriptLocation, rSave ), 0, 1 );
		...
		v[1] = VS( root, "K0" ) / v[0];
		v[2] = VS( root, "kFilledInit" ); 												// number of capital vintages to create to be != 0 capital quantity
		v[3] = VS( root, "gk0" );
		v[4] = 0;																		// aggregator
		...
		WRITELLS( cur1, "NominalCapitalStock", WHTAVES( cur1, "CapitalQuantity", "VintagePrice" ), 0, 1 );

		// normalise values (capital quantity) to total initial capital stock and fill technology variables (frontier and vintages' productivity)
		v[10] = SUMLS( cur1, "CapitalQuantity", 1 );
		v[11] = 0;
		v[12] = 1 / pow( 1 + v[3], 2);
		v[13] = imp_data( "A", scriptLocation, rSave );
		v[14] = MAXS( cur1, "CapitalQuantity" ) / v[10] * v[1];
		v[15] = v[1] * v[13] * ( 1 + v[3] ) / v[14] * ( v[12] - 1 ) / ( pow( v[12], v[2] )  - 1 );
		v[16] = 1;
		...
		v[18] = imp_data( "L", scriptLocation, rSave ) / v[0];						// total debt of each firm
		...
		CYCLES( cur, cur1, "Sectors" )
		{
			...
			ADDNOBJ_EXLS( cur1, "Firms", v[0], cur2, 0 );					// add identical firms in each sector
	...
	CYCLE( cur, "Countries" )
	{
		...
		CYCLES( cur, cur1, "Sectors" )
		{
			...
			{																// no
				plog( "\nERROR: program was terminated.\nNumber of firms per bank is not an integer.\nMake sure N Firms divided by N Banks is an integer in all sectors.");
					std::terminate();
			}
			v[14]= 0;
			v[15]= 1;
