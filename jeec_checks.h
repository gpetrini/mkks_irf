...
EQUATION( "CheckInventory" )
	...
	if ( abs( v[4] / v[5] ) > VS( root, "tol" ) ) 
		plog( "\nCheck of inventory accounting indicate some possible leakage. Please check." );
RESULT( v[4] )
...
/* Check whether capital stock accounting is correct.*/
	...
	{
		plog( "\nThere is possibly an accounting error in capital stock dynamics. Check causes. The error is equivalent to " );
...
/* Check accounting of firms' debt.*/
	...
	if ( abs( v[5] / v[3] ) > VS( root, "tol" ) && VS( root, "flagChecks" ) )
	{
		if ( v[3] != 0 )
		{
			plog( "\nThere is possibly an accounting error in firms' debt dynamics. Check causes. The error is equivalent to " );
