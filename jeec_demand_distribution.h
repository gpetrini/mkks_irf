...
/*
Average price for competitiveness. Not to be confounded with the effective price level,
which is obtained ex post and depends on current market share.
Level: sector.
*/
	v[0] = 0;								// aggregator
	CYCLE( cur, "Firms" )
...
*/
	...
	v[8] = v[3] == 0 ? -v[4] * v[6] : -v[4] * v[6] - v[5] * v[7]; // if the average unfilled demand is null, then disregard it in the competitiveness index.
RESULT( v[8] )
...
/* 
Function to recompute market share in case of entry/exit, change in some market share (near zero), or sum of market share != 1.
*/
	v[0] = SUM( "MarketShare" );
	CYCLE( cur, "Firms" )
	{
...
EQUATION( "Sales" )
/* 
Compute firm's sales. Updated afterwards if inventories are insufficient to fill existing demand for each firm's goods.
Level: firm.
*/
		V( "NearZeroMarketShare" );						// force to run after rebasing of market share was run once.
		v[0] = VS( GRANDPARENT, "AggregateDemand" );
		v[1] = V( "MarketShare" );
		v[2] = v[0] * v[1];
		WRITE( "FirmDemand", v[2] );					// demand faced by the firm. Used to compute production and investment plans		
...
/* 
Unfilled demand (quantity). Updated when relevant (sales and remaining sales distribution).
Level: firm.
*/ 
RESULT( 0 )	
...
/*
If there is a new entrant, total unfilled demand is calculated directly from the aggregates.
*/
		v[0] = SUM( "UnfilledDemand" );		
		v[1] = 0;
		CYCLE( cur, "Firms" )
		{
			v[1] += VS( cur, "EntryTime" ) == t - 1 ? 1 : 0;
		}
		if ( v[1] > 0 )
		{
...
*/
		...
		v[9] = SUM( "MarketShare" );
		if ( v[0] == 0 )								// if some firm could not fill all demand, rebase sales and recalculate market share. Otherwise, go ahead with the simulationion.
			END_EQUATION( 0 );
		v[1] = SUM( "Inventory" );
		if ( v[1] == 0 ) 
