...
/*
Updates the desired effective demand for labour, after the credit market interaction. 
Firms evaluate whether the funds are sufficient to produce the desired quantity.
Level: firm.
*/
	...
	if (  v[4] >= v[5] )											// are the deposits sufficient for hiring the desired amount of L?
	{																// yes
	...
	{
		v[7] = v[5] - v[4];												// haircut of desired payroll (total desired wage bill - funds available)
...
EQUATION( "WorkersRD" )
/*
Number of workers employed in the R&D sector. Applies to capital goods producing sectors. 
The Number of R&D workers is a function of nominal sales (R&D expenditure), deflated by the nominal wage. 
Level: firm.
*/
	v[0] = V( "RDExpenditure" );
	v[1] = V( "NominalWage" );
...
/*
Number of workers in production equals total number of workers the firm hired in the labour market, less the n_workers allocated in R&D.
Level: firm.
*/
	v[0] = V( "WorkersFirm" );
	v[1] = V( "WorkersRD" );
	if ( v[1] > v[0] )												// if the number of R&D workers is superior to the number of firm's workers, there was severe rationing. All workers goes to production.
	{
		WRITE( "WorkersRD", 0 );
		END_EQUATION( v[0] );
	}
...
Level: country.
*/
	if( t < 2 )																	// only works when there is a minimum amount of information in vector growthEmployment
		v[2] = 1;
	else
	{
		v[0] = ntile_vector( growthEmployment, V( "bandT" ) );					// get bandT %tile
		v[1] = VL( "GrowthRateEmployment", 1 );									
		v[2] = v[1] > v[0] ? 1 : 0;												// binary variable indicating whether last period's employment growth is greater than bandT %tile
	}
RESULT( v[2] )
...
*/
	...
	{	
		...
		v[1] = VL( "GrowthRateEmployment", 1 );
		v[2] = v[1] < v[0] ? -1 : 0;											// binary variable indicating whether last period's employment growth is lower than bandB %tile
	} 
RESULT( v[2] )	
