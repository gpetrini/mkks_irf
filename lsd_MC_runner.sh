...
while getopts "hd:b:u:M:p:l:r" option; do # read options included in the command line, create a list and loop through the list
  case $option in
...
if [ -f "$FOLDER/$LSDCONF.lsd" ] ; then
  printf 'Base file is %s and is locatd under %s, continuing.\n' "$LSDCONF.lsd" "$FOLDER"
