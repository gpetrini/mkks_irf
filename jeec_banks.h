/******************************************************************************
	...
	Few equations are added assuming that multiple bank object instances will
	be added in the future. However, the possibility of multiple banks still
...
It is assumed that firms holds all deposits in the last bank's that granted credit.
*/
	v[0] = 0;										
	v[1] = V( "idBank" );							// bank ID
	for( i = 0; i < COUNT_HOOK; i++ )
...
Only relevant when there is credit rationing.*/
	...
	{
		  v[0] = VLS( cur, "LoanShare", 1 ) != 0 ? VLS( cur, "LoanShare", 1 ) : 1;		// get asset share
		  WRITES( cur, "probDrawBank", v[0] );											// probability of being chosen depends on the market share
	}
RESULT( 0 )
...
/*
Amount of amortisation that the bank should receive from firms under the assumption of no default there is no default. 
*/
	v[0] = 0;
	v[1] = V( "idBank" );
