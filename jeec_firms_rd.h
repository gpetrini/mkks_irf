...
*/
		...
		v[4] = binomial( v[3], 1 );												// random draw of a binomial distribution, with probability of innovation given above	
		if ( v[4] )
		{
			v[5] = VL( "ACurrent", 1 );
			v[7] = 0;                                       					// Initialise sum of probabilities (for speed)
			v[6] = V( "idFirm" );
			CYCLES( PARENT, cur, "Firms" ) 										// compute the Euclidian distance between a firm best-known technologies to the others' techs
			{ 
				...
				v[11]= v[10] != 0 ?  1 / v[10] : 1e-40;							// invert scale of probability (the farther, the less likely to be randomly chosen).
				WRITES( cur, "probImitation",  v[11] );							// rewrite the probability of imitation
				v[7] += v[11];
			}
			cur = RNDDRAW_TOTS( PARENT, "Firms", "probImitation", v[7] );		// draw a firm to copy the technology
...
*/
		...
		v[7] = binomial( v[6], 1 );											// Draw of a binomial distribution, with the probability of innovation given above
		v[8] = beta( v[2], v[3] );											// Random draw of a beta distribution. Value may be either positive or negative
		v[9] = v[8] - v[2] / ( v[2] + v[3] );								// Normalise the draw to zero
		v[10]= v[9] * ( abs( v[4] ) + abs( v[5] ) );						// Rescale on support (betaMin, betaMax)
		v[11]= VL( "ACurrent" , 1);						
		v[12]= v[11] * ( 1 + v[7] * v[10] );								// New technology = old (best-known) technology times 1 + random draw.
...
*/
		...
		if ( v[0] == v[1] )												// If the firm is in the technological frontier, all the spending goes to innovation
		{
				v[4] = v[3];
		}
RESULT ( v[4] )	
...
EQUATION( "AvgImitationSuccess" )
	v[0] = 0;															// accumulator success weighted by id
	v[3] = COUNT( "Firms" );
	CYCLE( cur, "Firms" )
	{
