...
*/
	...
	v[1] = V( "PlannedInvestment" );									// total planned investment (expansion + replacement) 
	v[2] = V( "ExpectedWageBill" );										
	v[3] = VL( "FirmDeposits", 1 );										// begin-of-period deposits
	v[4] = max( 0, v[0] * v[1] + v[2] - v[3] );							// total demand for loans
	WRITE( "DemandLoansForBankRiskEvaluation", v[4] );					// write the parameter demand for loans that is used in bank's risk evaluation. This value is updated to disconsider investment in case the loan is not granted.
	if ( VS( GRANDPARENT, "flagRationing" ) )							// is the credit rationing flag activated?
	{																		// yes, find banks
		v[5] = VS( PARENT, "epsilon" );											// number of banks to search
		if ( v[5] > 1 )
		{
			v[6] = v[5] - 1;														// consider only from the second bank onwards
			while ( v[6] > 0 )											
			{																		// since firms try to keep relation with the same bank
				v[7] = V( "DrawABank" );
				cur = SEARCH_CND( "idBank", v[7] );
				WRITE_HOOK( v[6], cur );											// write hook pointer. Zero is the bank to which the firm is already client.
				v[6] -= 1;
			}
		}
	}
...
/*Calculate the probability of default of a loan project.*/
	...
	v[1] = VS( GRANDPARENT, "varPhi1" );
	v[7] = VS( GRANDPARENT, "varPhi2" );
	v[8] = VL( "Leverage", 1 );
	v[3] = VS( GRANDPARENT, "lambda" );															// duration of loan
	v[4] = V( "InterestAgreedBank" );															// get interest rate
	v[5] = v[2] / v[3] + v[4] * v[2];															// debt servicing of this loan project.
	v[6] = 1 / ( 1 + exp( v[1] * v[0] / v[5] - v[7] * v[8] ) );									// probability of default
RESULT( v[6] )

EQUATION( "ExpectedReturnOfLoanProject" )
	v[0] = V( "InterestAgreedBank" );
	v[1] = VS( GRANDPARENT, "lambda" );
	v[2] = V( "ProbabilityOfDefault" );
	if ( v[2] > 1e-04 )
		v[3] = 1 + v[0] * ( v[1] + 1 ) / 2 - ( 1 + v[0] ) * ( 1 + 1 / v[1] + ( pow( 1 - v[2], v[1] + 1 ) - 1 ) / ( v[1] * v[2] ) );
	else
		v[3] = 1 + v[0] * ( v[1] + 1 ) / 2;
RESULT( v[3] )	
...
/* 
Actual gross profits of firms, which here is defined as the operating cash flow less interest payments.
Level: firm.
*/
...
EQUATION( "NetProfits" )
/*
Actual net profits of firms. Duduction of the capital depreciation and inventory revaluation of the gross profits.
Level: firm.
*/
	v[0] = V( "NominalSales" );
	v[1] = V( "WageBill" );
...
*/
	...
	if ( v[3] > v[7] ) 										// firms won't pay dividends if they are illiquid
		v[3] = 0;
	v[8] = V( "flagExit" );
	if ( v[8] == 1 )										// firms won't pay dividends if they already know they are leaving the market.
		v[3] == 0;
RESULT( v[3] )	

EQUATION( "FirmDeposits" )
/*
Calculate the final firms' deposits, by deducing the amortisation and interest payments.
Level: firm. 
*/
	V( "DebtServicing" );									// run after debt servicing (because if firms are leaving the market amortisation and interest payments is updated there)	v[0] = V( "FirmDeposits_PostDividendsUpdate" );
...
It may differ from the actual wage bill if a firm is credit rationed and/or if it's labour rationed.
Level: firm.
*/
	v[0] = VS( GRANDPARENT, "NominalWage" );
...
*/	
	...
	cur = RNDDRAW( "Banks", "probDrawBank" );
...
Level: firm.
*/
	cur1 = SHOOK == NULL ? SEARCH_CND( "idBank", V( "BankSupplierID" ) ) : SHOOK;				// gather bank data
	WRITES( cur1, "DebtServicingRatio", VL( "DebtServicingToOperatingCashFlowRatio", 1 ) );		// pass information of lagged debt servicing to operating cash flow ratio (interest determination)
	v[0] = VS( cur1, "InterestRateBank" );														// get interest rate
RESULT( v[0] )
...
*/
	cur1 = SHOOK == NULL ? SEARCH_CND( "idBank", V( "BankSupplierID" ) ) : SHOOK;				// gather bank data
	v[0] = V( "InterestAgreedBank" );
	v[1] = VLS( cur1, "AverageBankInterestRate", 1 );
	v[2] = VS( GRANDPARENT, "lambda" );
	...
	{
		v[8] = v[0] * ( v[2] + 1 ) / 2 - ( 1 + v[0] ) * ( 1 + 1 / v[2] + ( pow( 1 - v[5], v[2] + 1 ) - 1 ) / ( v[2] * v[5] ) );
		v[9] = v[8] - v[1];										// error
		v[5] += v[9] * 0.05;
		v[6] = abs( v[9] );										// error variable update (absolute deviation from the target rate of return) 
		v[4]++;
	}
	if ( v[5] > V( "ProbabilityOfDefault" ) ) PLOG( "\nOpa! Deu ruim!" );
	v[10] = V( "ExpectedOperatingCashFlow" );
	v[11] = VS( GRANDPARENT, "varPhi1" );
	v[12] = VS( GRANDPARENT, "varPhi2" );
	v[13] = VL( "Leverage", 1 );
...
The result sums the deposits inherited from the past period plus the credit granted by the partner bank.
The creation of a loan vintage object storing the new loan information occurs only in equation CreateOutstandingLoan.
Level: firm.
*/
...
/*
This equation deducts wage payments from firms' deposits after the production takes place.
Level: firm.
*/
	v[0] = V( "FirmDeposits_PostCreditUpdate" );
	v[1] = V( "WageBill" );
...
*/
	...
	if ( v[3] > v[4] )															// in case the firm cannot pay the due taxes, it will exit the market.
	{
		v[3] = v[4];
		WRITE( "flagExit", 1 );
	}
...
/* 
Expected operating cash flow of firms. Assumed to be simply the last periods' cash flow.
Level: firm.
*/
...
That happens when the operating cash flow is greater than the investment plus the financial commitments.*/
	v[0] = V( "OperatingCashFlow" );
	v[1] = V( "Amortisation" );
	v[2] = V( "InterestPayment" );
...
That happens when the operating cash flow is less than the investment plus the financial commitments, 
but the cash flow is still greater than the financial commitments.*/
	v[0] = V( "OperatingCashFlow" );
	v[1] = V( "Amortisation" );
	v[2] = V( "InterestPayment" );
	v[3] = V( "NominalInvestment" );
...
That happens when the operating cash flow is less than the investment plus the financial commitments and the cash flow is still lower than the financial commitments.*/
	v[0] = V( "HedgeFirmFlag" );
...
Level: firm
*/
		// force execution at the bank level amortisation computation together with this equation.
		CYCLES( GRANDPARENT, cur, "Banks" )
		{
			VS( cur, "AmortisationDue" );
		}
...
*/
	...
	{
		cur1 = SHOOK == NULL ? SEARCH_CND( "idBank", V( "BankSupplierID" ) ) : SHOOK;	// search the supplier bank

		if ( VS( root, "flagFloatingInterestRate" ) ) 
			WRITE( "LoanInterestRate", VS( cur1, "InterestRateBank" ) );				// if the interest rate is floating, get bank's current rate
...
*/
	...
	v[1] = V( "FirmDeposits_PostDividendsUpdate" );						// deposits available at the liquidation stage. Needs to be updated.
	v[2] = v[0] > v[1] ? true : false;
	if ( v[2] ) WRITE( "flagExit", 1 );
...
b) in case the firm is able to repay amortisation and interest, write the information of debt-servicing in pertinent bank. 
Otherwise, firm defaults and dies (dealt with in another equation).
c) update time-management of loans information.
Level: firm.
*/

	v[0] = V( "DefaultFlag" );
	if ( !v[0] )																		// is the firm leaving the market? no. In case of default, the debt servicing is updated elsewhere.
		CYCLE( cur1, "OutstandingLoan" )
		{
			cur = SHOOK == NULL ? SEARCH_CND( "idBank", V( "BankSupplierID" ) ) : SHOOK;	// search the supplier bank

			v[1] = VS( cur, "InterestIncome" );												// get current interest income (bank)
			v[2] = VS( cur1,"InterestSchedule" );											// get interest schedule (firm)
			WRITES( cur, "InterestIncome", v[1] + v[2] );									// update interest income of bank with firm payment

			v[3] = VS( cur, "AmortisationFlow" );											// get amount of amortisation received by the bank
			v[4] = VS( cur1, "AmortisationSchedule" );										// get amortisation schedule
			WRITES( cur, "AmortisationFlow", v[3] + v[4] );									// update amortisation schedule 

			v[5] = VLS( cur1, "LoanOutstanding", 1 );										// get loan outstanding
			WRITES( cur1, "LoanOutstanding", v[5] - v[4] );									// update loan outstanding

			v[6] = VS( cur1, "PeriodsRemaining" );											// get number of periods remaining to fulfil the debt
			WRITES( cur1, "PeriodsRemaining", v[6] - 1 );		
		}
	// in case the firm is leaving the market (for any reason):
	// use existing deposits to pay banks (which is done proportionally, considering the amount owned per credit contract)
	// calculate total default
	// if there is some resources remaining in the bank account, transfer to households
	// this computation considers the credit supply of the current period, which still has not been added
	else														
	{
		...
		v[12]= v[10] + v[11];																
		v[14]= min( 1, v[12] != 0 ? v[9] / v[12] : 1 );											// proportion of interest that can be paid with existing deposits
		v[15]= min( 1, v[12] != 0 ? v[9] / v[12] : 1 );											// proportion of amortisation that can be paid with the existing deposits
		WRITE( "Amortisation", v[15] * v[10] );													// rewrite actual amortisation actually paid by the firm.
		v[13]= V( "CreditSupply" );															
		v[16]= v[13] * v[15];																	// accumulator of amount paid
		v[17]= V( "CreditSupply" ) - v[16];														// accumulator of firms' default
		v[9]-= v[16];																			// exclude payment of current period's debt out of available resources
		cur = SEARCH_CND( "idBank", V( "BankID" ) );
		WRITES( cur, "AmortisationFlow", VS( cur, "AmortisationFlow" ) + v[16] );			 	// update amortisation flow of bank that granted credit this period
		WRITES( cur, "NonPerformingLoans", VS( cur, "NonPerformingLoans" ) + v[17] );						// update default of the bank
		...
		CYCLE( cur1, "OutstandingLoan" )
		{
			if ( v[9] > 0 )
			{
				...
				WRITES( cur, "InterestIncome", v[1] + v[3] );									// update interest income of bank with firm payment

				
				v[4] = VS( cur, "AmortisationFlow" );											// get amount of amortisation received by the bank
				v[5] = VS( cur1, "LoanOutstanding" );											// get loan outstanding
				v[6] = v[5] * v[15];															// amortisation due (apply % of total loans outstanding)
				v[9] -= v[6];																	// reduce available deposits after amortisation
				v[8] = v[5] - v[6];																// non-performing loans of banks (includes write-off of loans)
				v[16]+= v[6];																	// accumulate total amount paid
				v[17]+= v[8];																	// accumulate total default (include interest)
				WRITES( cur, "AmortisationFlow", v[4] + v[6] );									// update amortisation flow of bank 
				
				
				v[7] = VS( cur, "NonPerformingLoans" );	
				v[18]= v[8] + v[2] - v[3];														// default (include interest not paid to banks)
				
				if ( v[8] > 0 ) 
				{
					WRITES( cur, "NonPerformingLoans", v[7] + v[8] );							// update total non-performing loans of bank (does not include interest)
					WRITES( cur, "DefaultLoans", VS( cur, "DefaultLoans" ) + v[18] );     		// update total default on loans
				}
		...
		if( v[9] > 0 ) 																			// if there is some remaining cash balance, distribute between households
		{
			v[18] = COUNTS( GRANDPARENT, "Households" );
			CYCLES( GRANDPARENT, cur2, "Households" )
			{
				WRITES( cur2, "CapitalContribution", VS( cur2, "CapitalContribution" ) - v[9] / v[18] );
				WRITE( "FirmDeposits_PostDividendsUpdate", V( "FirmDeposits_PostDividendsUpdate" ) - v[9] / v[18] ); // deduce deposits of this transfer to households
			}
		}
		RECALC( "GrossProfits" );																// consider the changed interest payments into the gross and net profits
		RECALC( "NetProfits" );								
...
/* 
This equation eliminates expired outstanding loans instances. It further creates new loans object after the credit market interaction.
Created after debt servicing.
Level: firm.
*/
	...
	CYCLE_SAFE( cur, "OutstandingLoan" )
	 { // for each OutstandingLoan
