...
WRITES(cur1, "DebtServicingRatio",
...
// equation.
CYCLES(GRANDPARENT, cur, "Banks") { VS(cur, "AmortisationDue"); }
...
CYCLE(cur, "OutstandingLoan") {
  cur1 = SHOOK == NULL ? SEARCH_CND("idBank", V("BankSupplierID"))
                       : SHOOK; // search the supplier bank

  if (VS(root, "flagFloatingInterestRate"))
    WRITE("LoanInterestRate",
          VS(cur1, "InterestRateBank")); // if the interest rate is floating,
...
// has not been added
  ...
  WRITES(cur, "AmortisationFlow",
         VS(cur, "AmortisationFlow") +
             v[16]); // update amortisation flow of bank that granted credit
                     // this period
  WRITES(cur, "NonPerformingLoans",
         VS(cur, "NonPerformingLoans") + v[17]); // update default of the bank
  ...
  CYCLE(cur1, "OutstandingLoan") {
    if (v[9] > 0) {
      ...
      v[4] = VS(cur, "AmortisationFlow"); // get amount of amortisation received
                                          // by the bank
                ...
                // households
  {
    ...
    CYCLES(GRANDPARENT, cur2, "Households") {
      WRITES(cur2, "CapitalContribution",
             VS(cur2, "CapitalContribution") - v[9] / v[18]);
