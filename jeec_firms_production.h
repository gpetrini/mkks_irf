...
Since while firms fix prices firms do not know the actual production, consider
the ex ante one to compute the average labour productivity. Level: firm.*/
v[0] = V("Markup");
v[1] = V("NominalWage");
...
/*
Desired level of capacity utilisation. May be higher than 1 (important for
expansionary investment behaviour). Bounded later in actual capacity utilisation
equation. Level: firm.
*/
v[0] = VS(PARENT, "nu");       // Capital output ratio (parameter)
v[1] = V("DesiredProduction"); // Desired production or orders
v[2] =
    V("AvailableCapitalStock"); // Total capital stock available for production
v[3] = v[0] * v[1] / v[2];      // Desired capacity utilisation
RESULT(v[3])

EQUATION("DesiredProduction")
/*
Desired Production. Depends on the expected sales and on the opening
inventories. Level: firm.
*/
...
(which may differ from the actual sales, if the firm could not fill the entire
demand). Level: firm.
*/
