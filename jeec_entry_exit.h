...
CYCLE(cur, "Firms") {
  ...
  if (VS(cur, "flagExit") == 1) {
    ...
    WRITE("AggregateNominalDepreciation",
          V("AggregateNominalDepreciation") + v[3]);
    WRITE("AggregateInventories",
          V("AggregateInventories") - VS(cur, "Inventory"));
...
if (LAST_T - t < 2)
  END_EQUATION(0); // only run equation until two steps before the simulation is
                   // stopped (entry routine requires that)
...
for (i = 0; i < v[0]; i++) {
  ...
  // firms have initially zero debt (number of objects in excess are deleted in
  // the next period in Outstanding Loan object's housekeeping
  ...
  // firms have initially zero capital stock (investment on new capital goods
  // are stablished later)
  CYCLES(cur, cur1, "CapitalVintage") {
  ...
  // initial market share of firm (starting from two periods ahead) starts from
  ...
  // initial expected sales is related to the initial capital stock. Fill values
  // ahead too.
  ...
  // the other part of the resources to stablish a new firm comes from new
  // loans. to define the proportion of capital contribution by households and
  // new debt, we consider the average leverage of the firms' sector and the
  // average liquid assets to total assets of the firms' sector. this design is
  // inteded to affect the least model's results through entry.
  v[13] = V("AggregateFirmDeposits") /
          V("AggregateAssetsFirms"); // average liquidy ratio
  v[14] = V("AggregateLeverage");
  v[15] = v[8] * V("iota"); // expected inventory (quantity)
  v[16] = v[15] * V("PriceLevel") /
          (1 + V("AverageMarkup"));     // expected inventory (nominal)
  ...
  v[19] = v[17] + v[8] * V("PriceLevel") -
          v[18]; // total deposits needed by the new firm (includes initial
  ...
  WRITES(PARENT, "ChangeDepositsFirmsSector",
         VS(PARENT, "ChangeDepositsFirmsSector") + v[19]);
  WRITES(PARENT, "AggregateFirmDeposits",
         VS(PARENT, "AggregateFirmDeposits") + v[19]);

  // expected cash flow of firms (needed for loan demand when there is credit
  // rationing)
  v[11] = (V("initialMarkup") - V("gamma")) * V("NominalWage") * v[8] /
          v[3]; // expected nominal profits minus the wage bill
