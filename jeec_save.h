...
EQUATION("__WeiItoK")
RESULT(V("AvailableCapitalStock") > 0
           ? V("ActualInvestment") * V("CapitalShare") /
                 V("AvailableCapitalStock")
           : 0)
...
RESULT(V("AvailableCapitalStock") > 0
           ? V("OperatingCashFlow") / VL("NominalCapitalStock", 1)
           : 0)
...
v[0] = 0;
CYCLE(cur, "Sectors") {
  CYCLES(cur, cur1, "Firms") { v[0] += pow(VS(cur1, "MarketShare"), 2); }
}
RESULT(v[0])
